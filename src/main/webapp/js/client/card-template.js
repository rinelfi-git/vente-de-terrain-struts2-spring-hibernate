...
function clientCardTemplate(id, nom, prenom, cin, adresse, telephones, photo) {
    ...
    clientCardTemplate += '<a href="javascript:;" class="dropdown-item" data-toggle="modal" data-target="#modification-modal" onclick="initialiserModifier(' + id + ')"><span class="material-icons">update</span> modifier</a>'
    clientCardTemplate += '<div class="dropdown-divider"></div>'
    clientCardTemplate += '<a href="javascript:;" class="dropdown-item" onclick="promptDelete(' + id + ', \'' + cin + '\')"><span class="material-icons">delete_forever</span> supprimer</a>'
    clientCardTemplate += '</div>'
    clientCardTemplate += '</div>'
    clientCardTemplate += '</div>'
    clientCardTemplate += '</div>'
